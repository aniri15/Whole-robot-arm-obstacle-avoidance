...
def nullspace_method(jac_joints, delta, regularization_strength=0.0):
  ...
  """
  hess_approx = jac_joints.T.dot(jac_joints) # J.T * J
  joint_delta = jac_joints.T.dot(delta) # J.T * delta
  if regularization_strength > 0:
    # L2 regularization
    hess_approx += np.eye(hess_approx.shape[0]) * regularization_strength
    return np.linalg.solve(hess_approx, joint_delta) # (J.T * J + λI)^-1 * J.T * delta
  else:
