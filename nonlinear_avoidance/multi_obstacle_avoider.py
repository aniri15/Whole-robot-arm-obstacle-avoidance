...
def compute_multiobstacle_relative_velocity(
    ...
    for ii, obs in enumerate(environment):
        ...
        #if obs.twist is not None:
           # print("linear", obs.twist.linear)
            #print("angular", obs.twist.angular)
...
class MultiObstacleAvoider:
    ...
    def evaluate_sequence(self, position: Vector) -> Vector:
        ...
        goal = self.initial_dynamics.attractor_position
        if np.linalg.norm(position - goal) < 0.1:
            slowed_velocity = slowed_velocity * 1.5
        if False:
        ...
        # breakpoint()
        #print("slowed veclocity ", slowed_velocity)
        #print("--------------------------")
        return slowed_velocity + relative_velocity
    
    def evaluate_sequence_norm(self, position: Vector) -> Vector:
        if hasattr(self.initial_dynamics, "evaluate_magnitude"):
            initial_magnitude = self.initial_dynamics.evaluate_magnitude(position)
        ...
        #length = len(self.obstacle_container)
        if not len(self.obstacle_container):
            return initial_sequence.get_end_vector() * 0
    ...
    def evaluate_avoidance_from_sequence(self, position, initial_sequence, normal = False):
        ...
        #print("maximum: ", max(self._cluster_weights))
    ...
    def simple_population_tangent_tree(
        ...
        if not normal:
            ...
            self.convergence_radius = math.pi
            convergence_radiuses = self.compute_convergence_radiuses(
                gamma_values, position=position, obstacle_tree=obstacle
            )
    ...
    def _simple_tangent_branch_update(
        ...
        if (
            ...
            if not normal:
                ...
                avoidance_direction = normal_directions[0]
        else:
