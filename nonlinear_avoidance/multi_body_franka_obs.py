...
#from envs.franka_test_roam_env import FrankaTestEnv
...
def set_env(env_name: str, num_obst: int, control_mode: str, scenario_name: str) -> gym.Env:
    register_custom_envs()
    # transfer the information to the environment (e.g. number of obstacles)
    env = gym.make(env_name, num_obst= num_obst, n_substeps=20, control_mode=control_mode, scenario=scenarios[scenario_name])
    return env
...
def create_3d_franka_obs(FrankaEnv):
    obstacles = FrankaEnv.get_obstacle_info()
    print("obstacles", obstacles)
    obs_pos = np.zeros((len(obstacles), 3))
    obs_size = np.zeros((len(obstacles), 3))
    # moving obstacles
    for i in range(len(obstacles)):
        obs_pos[i] = obstacles[i]['pos']
        obs_size[i] = obstacles[i]['size']
...
def create_3d_franka_obs2():
    drawer_center1 = [0.3,0.5,0.45]
    drawer_center2 = [0.3,0.3,0.45]
    drawer_center3 = [0.405,0.4,0.45]
    drawer_center4 = [0.195,0.4,0.45]

    drawer_size12 =  [0.1,0.001,0.05]
    drawer_size34 = [0.001,0.1,0.05]
   

    moving_obstacle_center = [0.4,0.3,0.12]  #[0.7,0.6,0.42]
    moving_obstacle_size = [-0.1,-0.1,-0.1]  #[0.2,0.2,0.2]
    ...
    new_human.set_root(
        Cuboid(
            axes_length=drawer_size12,
            center_position=drawer_center1,
            distance_scaling=distance_scaling,
            margin_absolut=margin_absolut,
        ),
        name="body",
    ...
    # for i in range(10):
    #     print("i",i )
    #     new_human[-1].set_reference_point(np.array([0+0.01*i,0,0]), in_global_frame=False)
    #     #new_human[-1].set_reference_point(np.array([0,0+0.01*i,0]), in_global_frame=False)
        
    new_human[-1].set_reference_point(np.array([0,0,0]), in_global_frame=False)

    new_human.add_component(
        Cuboid(
            # axes_length=np.array([0.12, 0.2, 0.12]),
            axes_length=drawer_size12,
            center_position=drawer_center2,
            distance_scaling=distance_scaling,
            margin_absolut=margin_absolut,
        ),
        name="neck",
        parent_name="body",
        reference_position=np.array([0.01,0,0]),
        parent_reference_position=np.array([0.0, 0, 0.30]),
    )

    new_human.add_component(
        Cuboid(
            axes_length=drawer_size34,
            center_position=drawer_center3,
            distance_scaling=distance_scaling,
            margin_absolut=margin_absolut,
        ),
        name="head",
        parent_name="neck",
        reference_position=np.array([0.0, 0.02, 0]),
        parent_reference_position=np.array([0.0, 0, 0.05]),
    )
    ...
    new_human.add_component(
        Cuboid(
            axes_length=drawer_size34,
            center_position=drawer_center4,
            distance_scaling=distance_scaling,
            margin_absolut=margin_absolut,
            orientation=Rotation.from_euler("xyz", [0, 0.1 * np.pi, -0.3 * np.pi]),
        ),
        name="upperarm1",
        parent_name="body",
        reference_position=np.array([0.0, 0.03, 0]),
        parent_reference_position=[0.15, 0, 0.3],
    )
...
if __name__ == '__main__':
    # env = set_env("FrankaTestEnv-v0")
    env = set_env("FrankaTestEnv",3,"ik_controller",'lift_maze')
    env.reset()

    #obs = create_3d_franka_obs(env)
    obs = create_3d_franka_obs2()
    

    from scripts.three_dimensional.human_avoidance import Visualization3D
    visualization = Visualization3D()
    visualization.plot_obstacles(obs)
    # human = create_2d_human()
    # human.update()

    # human = create_2d_human_faulty()
    # human.update()

    # human = create_2d_human_with_multib