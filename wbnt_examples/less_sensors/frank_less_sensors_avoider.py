...
#----------------------------------------------------------------------------------------------------------
    ...
    def update_step(self, ii: int) -> None:
        ...
        for it_traj in range(self.n_traj):
            ...
            if self.obstacle:
                ...
                distances_length = len(self.container.get_tree(0))+1
                #print("distances_length: ", distances_length)
                if len(weights_ee) == distances_length:
                    self.weight_ee.append(weights_ee)
                else:
                    reshaped_weights = np.zeros(distances_length)
    ...
    def update_ee_step(self, ii: int) -> None:
        ...
        #     if self.obstacle:
        #         distances_sensor = self.avoider.get_final_distances_for_sensors()
        #         for num in range(len(self.container)):
        #             distances_length += len(self.container.get_tree(num))
        #         distances_length += 1
        #         #print("distances_length: ", distances_length)
        #         if len(distances_sensor) == distances_length:
        #             self.distance_sensors.append(distances_sensor)
        #         elif len(distances_sensor) != 1 and len(distances_sensor) < distances_length:
        #             reshaped_weights = np.zeros(distances_length)
        #             #reshaped_weights[:len(distances_sensor)] = distances_sensor
        #             reshaped_weights[distances_length-len(distances_sensor):] = distances_sensor
        #             self.distance_sensors.append(reshaped_weights)
        #         else:
        #             reshaped_weights = np.zeros(distances_length)
        #             reshaped_weights[:len(distances_sensor)] = distances_sensor
        #             self.distance_sensors.append(reshaped_weights)
        #     else:
        #         self.distance_sensors.append(np.zeros(1))
