...
class FrankaHumanEnv():
    ...
    def get_joints_qpos(self, joints_idex, joints_goal, joints_vel):
        joints_id = []
        for i in range(1,len(self.rotation_joints)):
            k = self.rotation_joints[i]
            joints_id.append(self.model.body(f'link{k}').id)
    ...
    def maximize_q_vel(self,avoid_vel,ref_vel, body_id):
        # sensors_jac = self.get_sensors_jac()
        # #print("sensors_jac: ", sensors_jac.shape)
        # m,_,_ = sensors_jac.shape
        # sensors_q_vel_max = []
        # sensors_q_vel = []

        # # get the jacobian of the end effector and reference joint angles
        # ee_jac = np.zeros((6, self.model.nv))
        # mujoco.mj_jac(self.model, self.data, ee_jac[:3], ee_jac[3:], self.data.body(body_id).xpos, body_id)
        # dv = self.desired_velocity(ref_vel)
        # n = ee_jac.shape[1]
        # I = np.identity(n)
        # product = ee_jac.T @ ee_jac + self.damping * I
        
        # if np.isclose(np.linalg.det(product), 0):
        #     j_inv = np.linalg.pinv(product) @ ee_jac.T
        # else:
        #     j_inv = np.linalg.inv(product) @ ee_jac.T
        ...
        # for i in range(m):
            ...
            # q_vel = j_inv_sensor @ avoid_vel[i]
    
            # sensors_q_vel_max.append(np.linalg.norm(q_vel - ref_q_vel))
            # sensors_q_vel.append(q_vel)

        #print("weights_sensors: ", len(self.animator.weights_sensors))
        #weights_sensors = np.array(self.animator.weights_sensors)
        weights_sensors = self.animator.weights_sensors
        sensors_q_vel_max = []
        sensors_q_vel = []
        ...
        if np.any(weights_sensors_obstacles > 0.8):
            ...
            sensors_index = index - (link_idex * num_sensors_link)
            link_sensor = self.link_names[link_idex]
            jac = self.get_required_sensor_jac(link_sensor, link_idex, sensors_index)
        ...
        #     weights_sensors_obstacles = weights_sensors[i][:-1]
        #     weights_sensors_convergence_dynamics = weights_sensors[i][-1]
        #     if np.any(weights_sensors_obstacles > 0.8):
        #         # avoid the obstacles using the avoiding velocity
        #         num_sensors_link = self.total_sensors // len(self.link_names)
        #         link_idex = i // num_sensors_link
        #         sensors_index = i - (link_idex * num_sensors_link)
        #         link_sensor = self.link_names[link_idex]
        #         jac_sensors = self.get_required_sensor_jac(link_sensor, link_idex)
        ...
        #         sensors_q_vel_max.append(np.linalg.norm(q_vel))
        #         sensors_q_vel.append(q_vel)

        # sensors_q_vel_max = np.array(sensors_q_vel_max)
        # maximum_idx = np.argmax(sensors_q_vel_max)
        # return sensors_q_vel[maximum_idx]
    ...
    def MPC_control(self, goal, velocity):
        pass
    ...
    def move_point(self,init_position, goal):
         #self.mujoco_renderer.render(self.render_mode)
        it_max = self.it_max
        self.animator = MayaviAnimator(it_max=it_max, end_effector_position=init_position, attractor_position=goal)    
        self.animator.setup()
        self.human_obstacle = self.animator.human_obstacle_3d
        self.adjust_obstacle_init()
        self.animator.dynamic_human = self.dynamic_human
    ...
    def move_franka_robot_sample_based(self,init_position, goal):
        #self.mujoco_renderer.render(self.render_mode)
        it_max = self.it_max
        ...
        for ii in range(it_max):
            ...
            for jj in range(len(velocity)):
                joint_next_desired_pos = sampled_points[jj] + velocity[jj]* 0.05#self.delta_time  #self.step_size_position
                next_desired_pos.append(joint_next_desired_pos)
            #self.use_ik_controller(next_desired_pos)
            #self.panda_ik_controller(next_desired_pos)
            # append the first joint position at the beginning
            next_desired_pos.insert(0, init_position[0])
            feasible_results = self.search_feasible_solutions(next_desired_pos)
            # if feasible_results:
            #     for i in range(len(feasible_results)):
            #         self.joints_resolved_rate_motion_control(feasible_results[i], velocity)
            if feasible_results:
                self.joints_resolved_rate_motion_control(init_position,feasible_results, velocity)
            else:
                print("no feasible solutions")
                self.replay()
                self.MPC_control(next_desired_pos, velocity)
    ...
    def sample_multiple_points(self, initial_positions, num_points, sample_range):
        # Trajectory integration
        #print("initial_positions: ", initial_positions.shape)
        print("initial_positions: ", initial_positions.shape)
        self.sampled_number_points = num_points * num_points
        start_positions_list = np.zeros(((self.rotation_joints.shape[0])*(num_points)**2, 3))
        #print("start_positions_list shape: ", start_positions_list.shape)
        for i in range(initial_positions.shape[0]):
            x_range = [initial_positions[i][0] + sample_range, initial_positions[i][0] - sample_range]
            #y_value = initial_positions[i][1]
            y_value = [initial_positions[i][1] + sample_range, initial_positions[i][1] - sample_range]
            z_range = [initial_positions[i][2] + sample_range, initial_positions[i][2] - sample_range]
            
            #yv = y_value * np.ones(num_points * num_points)
            yv = np.linspace(y_value[0], y_value[1], num_points*num_points)
            xv, zv = np.meshgrid(
                np.linspace(x_range[0], x_range[1], num_points),
                np.linspace(z_range[0], z_range[1], num_points),
            )

            start_positions = np.vstack((xv.flatten(), yv.flatten(), zv.flatten()))
            start_positions_list[i*num_points*num_points:(i+1)*num_points*num_points,:] = start_positions.T
        
        print("start_positions_list shape: ", start_positions_list.shape)

        return start_positions_list
    
    def store_link_length(self):
        import xml.etree.ElementTree as ET

        # Load the XML file
        #tree = ET.parse('/home/aniri/Master_related/MuJoCo-Tutorial/xml/panda.xml')
        tree = ET.parse('/home/aniri/nonlinear_obstacle_avoidance/franka_gym_test/envs2/franka_emika_panda/panda.xml')
        root = tree.getroot()
        #r = R.from_euler('xyz', [0, 0, 0], degrees=True)
        # Get the positions of the links
        joints = self.rotation_joints
        #joints = np.array([0,1,2,3,4,5,6,7])
        dimensions = 3
        #positions = np.zeros((joints.shape[0]+1, dimensions))
        positions = np.zeros((len(joints), dimensions))
        rotation_matrix = []
        link_lengths = []
        ...
        for j in range(0, len(joints)):
            ...
            for body in root.findall(".//body"):
                pos = body.get("pos")
                if pos and body.get("name") == f"link{i}":
                    positions[j] = self.get_body_xpos(f"link{i}")
                    #print(f"Link {i} position: {positions[j]}")
                    break
                # if not found, then add [0, 0, 0]
                if body == root.findall(".//body")[-1]:
                    positions[j] = [0, 0, 0]

        #positions[-1] = self.get_ee_position()
        #print("positions:", positions)
        for i in range(1, len(positions)):
            link_length = np.linalg.norm(np.array(positions[i])-np.array(positions[i-1]))
            link_lengths.append(link_length)

        #print("positions:", positions)
        print("Link lengths:", link_lengths)
        print("Link lengths sum:", np.sum(link_lengths))

        self.link_length_storage  = link_lengths
    
    def search_feasible_solutions(self, franka_joint_positions):
        # check if the joint positions satisfy the joint link lengths
        feasible_joint_pairs = []
        feasible_solutions = []
        fixed_joint_positions = franka_joint_positions[0]
        franka_joint_positions = franka_joint_positions[1:]

        # i = 0, (0, [0:25]) joint1, 3
        feasible_pairs = []
        for i in range(self.sampled_number_points):
            link_length = np.linalg.norm(fixed_joint_positions - franka_joint_positions[i])
            if np.linalg.norm(link_length - self.link_length_storage[0]) < self.length_tol:
                feasible_pairs.append((fixed_joint_positions, franka_joint_positions[i]))
        feasible_joint_pairs.append(feasible_pairs)
        #print("feasible_joint_pairs: ", feasible_pairs)
        #print("length", len(feasible_pairs))
        #print("-----------------")


        # i = 0 [0:25],[25:50] joint 3, 5
        # i = 1 [25:50],[50:75] joint 5, end effector
        for i in range(self.rotation_joints.shape[0]-1):
            feasible_pairs = []
            for j in range(i*self.sampled_number_points, (i+1)*self.sampled_number_points):
                for k in range((i+1)*self.sampled_number_points, (i+2)*self.sampled_number_points):
                    link_length = np.linalg.norm(franka_joint_positions[j] - franka_joint_positions[k])
                    if np.linalg.norm(link_length - self.link_length_storage[i+1]) < self.length_tol: 
                        feasible_pairs.append((franka_joint_positions[j], franka_joint_positions[k]))
            feasible_joint_pairs.append(feasible_pairs)
            #print("feasible_joint_pairs: ", feasible_pairs)
            #print("length", len(feasible_pairs))
            #print("-----------------")

        #print("feasible_joint_pairs: ", feasible_joint_pairs)
        for i in range(1, len(feasible_joint_pairs)-1):  
            new_chains = []
            for j in range(len(feasible_joint_pairs[i-1])):
                j1, j2 = feasible_joint_pairs[i-1][j]
                for k in range(len(feasible_joint_pairs[i])):
                    k1, k2 = feasible_joint_pairs[i][k]
                    for m in range(len(feasible_joint_pairs[i+1])):
                        m1, m2 = feasible_joint_pairs[i+1][m]
                        if np.all(j2 == k1) and np.all(k2 == m1):
                            new_chains.append((j1, j2, k2, m2))
                            #print("new chains: ", (j1, j2, k2, m2))
                            #print("link_length: ", np.linalg.norm(j2-j1), np.linalg.norm(j2 - k2), np.linalg.norm(k2 - m2))
                        #new_chains.append((j1, j2, k2))
            #feasible_chains.append(new_chains)
        feasible_solutions = new_chains
        
        if len(feasible_solutions) > 0:
            return feasible_solutions
        return None    

    def search_feasible_solutions2(self, franka_joint_positions):
        # check if the joint positions satisfy the joint link lengths
        feasible_joint_pairs = []
        feasible_solutions = []
        fixed_joint_positions = franka_joint_positions[0]  # joint 1 position 
        franka_joint_positions = franka_joint_positions[1:] # joint 3, 5, end effector positions and the sampled points around them
        tot_sampled_points = self.sampled_number_points **2
        # if the sample points are 5
        # i = 0, (0, [0:25]) joint1, 3
        
        mean_joint_position_3 = np.mean(franka_joint_positions[:tot_sampled_points], axis=0)
        mean_joint_position_5 = np.mean(franka_joint_positions[tot_sampled_points:2*tot_sampled_points], axis=0)
        #mean_joint_position_ee = np.mean(franka_joint_positions[2*tot_sampled_points:,:], axis=0)

        joint_3_position = fixed_joint_positions + self.link_length_storage[0]*(mean_joint_position_3 / np.linalg.norm(mean_joint_position_3))
        joint_5_position = joint_3_position + self.link_length_storage[1]*(mean_joint_position_5 / np.linalg.norm(mean_joint_position_5))
        ee_position = franka_joint_positions[-1]
        # i = 0 [0:25],[25:50] joint 3, 5
        # i = 1 [25:50],[50:75] joint 5, end effector
        feasible_joint_pairs.append((fixed_joint_positions, joint_3_position,joint_3_position, joint_5_position, ee_position))
        feasible_solutions.append(feasible_joint_pairs)
    ...
    def get_sensor_position(self):
        ...
        for _, name in enumerate(self.link_names):
            ...
            for i in range(sensors_on_link):
                ...
                sensors_ee_pos[i] = np.array([sensor_pos[0], sensor_pos[1], sensor_pos[2]])
