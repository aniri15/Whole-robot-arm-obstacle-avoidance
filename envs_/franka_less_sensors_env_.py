...
from extended_roam_examples.less_sensors.frank_less_sensors_avoider import ROAM_distance
import os
...
class FrankaLessSensorsEnv_():
    ...
    def move_robot(self,init_position, goal):
        ...
        it_max = self.it_max
        self.animator = ROAM_distance(it_max=it_max, current_position=init_position, 
    ...
    def move_franka_robot_norm_dir(self,init_position, goal):
        ...
        self.animator = ROAM_distance(it_max=it_max, current_position=sampled_points, 
                                  attractor_position=goal, dynamic_human=self.dynamic_human, obstacle=self.obstacle)    
    ...
    def maximize_q_vel(self,avoid_vel,ref_vel, body_id):
        ...
        weights_sensors = self.animator.distance_sensors
        ...
        if np.any(weights_sensors_obstacles < 1.6): #0.5
            index = np.argmin(weights_sensors_obstacles)
            index = index // weights_sensors_obstacles.shape[1]
            obstacle_id = np.argmin(weights_sensors_obstacles[index])
            print("indx: ", index)
            ...
            print("obstacle id",obstacle_id,
                  "maximum value: ",np.min(weights_sensors_obstacles[index]))
